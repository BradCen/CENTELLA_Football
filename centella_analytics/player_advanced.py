...
def player_similarity(passports: dict[str, dict], player_id: str | None = None, top_k: int = 5) -> dict:
    ...
    features = (
        ...
        ("physical", "high_speed_share"),
        ("physical", "sprint_share"),
        ("physical.position_span", "x"),
        ("physical.position_span", "y"),
        ("technical_per_90", "passes_attempted"),
        ("technical_per_90", "progressive_pass_distance_m"),
        ("technical_per_90", "shots"),
        ("technical_per_90", "duels_won"),
    )

    def feature_value(passport: dict, section: str, key: str) -> float:
        value = passport
    ...
    ids = list(passports)
    X = np.asarray([[feature_value(passports[pid], s, k) for s, k in features] for pid in ids], float)

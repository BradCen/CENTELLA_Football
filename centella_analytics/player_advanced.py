...
def player_evolution(frames: Sequence[TrackingFrame], player_id: str, events=(), windows: int = 4) -> dict:
    ...
    for i, chunk in enumerate(chunks):
        ...
        snapshots.append({"window": i, "start_t": float(t0), "end_t": float(t1), "samples": len(vals), "mean_speed_mps": float(speeds.mean()), "max_speed_mps": float(speeds.max()), "mean_x_m": float(np.mean([p.x for p in vals])), "mean_y_m": float(np.mean([p.y for p in vals])), "sprint_share": float(np.mean(speeds >= 7.0)), "passes_per_90": float(90.0 * len(passes) / mins), "shots_per_90": float(90.0 * len(shots) / mins)})
...
def player_similarity(passports: dict[str, dict], player_id: str | None = None, top_k: int = 5) -> dict:
    ...
    features = (("physical", "mean_speed_mps"), ("physical", "max_speed_mps"), ("physical", "high_speed_share"), ("physical", "sprint_share"), ("physical", "coverage_span_x_m"), ("physical", "coverage_span_y_m"), ("technical_per_90", "passes_attempted"), ("technical_per_90", "progressive_pass_distance_m"), ("technical_per_90", "shots"), ("technical_per_90", "duels_won"))
    ids = list(passports)
    ...
    Z = (X - mu) / sig
    def nearest(i):
        d = np.linalg.norm(Z - Z[i], axis=1); order = np.argsort(d)
        return [{"player_id": ids[j], "distance": float(d[j]), "similarity_pct": float(100.0*np.exp(-d[j]))} for j in order if j != i][:top_k]
    if player_id is None: groups = {pid: nearest(i) for i, pid in enumerate(ids)}
    elif player_id in passports: groups = {player_id: nearest(ids.index(player_id))}
    else: return {"valid": False, "reason": "unknown_player_id", "player_id": player_id}
...
def contribution_profile(frames: Sequence[TrackingFrame], player_id: str, events=(), team: str | None = None, radius_m: float = 12.0) -> dict:
    ...
    samples = [p for f in frames for p in f.players if p.player_id == player_id]
    if not samples: return {"valid": False, "reason": "no_player_samples", "player_id": player_id}
    ...
    for f in frames:
        ...
        if p is None or f.ball is None: continue
    ...
    return {"valid": True, "player_id": player_id, "team": team, "on_ball": {"passes": len(passes), "receipts": len(receipts), "shots": len(shots), "duels": len(duels), "observed_ball_proximity_rate": float(np.mean(near_ball)) if near_ball else None}, "off_ball": {"mean_near_teammates_within_radius": float(np.mean(support)) if support else None, "spatial_observations": len(samples), "mean_x_m": float(np.mean([p.x for p in samples])), "mean_y_m": float(np.mean([p.y for p in samples])), "movement_span_x_m": float(max(p.x for p in samples)-min(p.x for p in samples)), "movement_span_y_m": float(max(p.y for p in samples)-min(p.y for p in samples))}, "provenance": "observed tracking/event contribution proxies; not causal player-value attribution"}

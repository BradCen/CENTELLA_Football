...
def build(paths):
    ...
    models={c:v10.seed_model(c) for c in paths}; keep={c:KeepAll() for c in paths}
    frames=[]; stats=defaultdict(list)
    for i,t in enumerate(times):
        ...
        for d in fu:
            # Preserve a scalar node score; fusion may combine several cameras.
...
def edge_cost(a,b,dt):
    dist=float(np.linalg.norm(np.asarray(a['xy'])-np.asarray(b['xy'])))
    speed=dist/max(dt,.04)
    if speed>MAX_SPEED:return None
    # Normalize motion by a soft expected per-frame displacement.  Appearance
    # can break close spatial ties but cannot rescue an impossible jump.
    motion=dist/max(.75,MAX_SPEED*dt)
    app=max(0.,app_cost(a,b))
    node=max(0.,float(b.get('node_score',0.)))
...
def best_path(frames,blocked):
    ...
    for ti,f in enumerate(frames):
        for di,d in enumerate(f['fused']):
            if (ti,di) not in blocked:
                nodes.append((ti,di,d))
    if not nodes:return []
    ...
    for ti in sorted(by_t):
        for k in by_t[ti]:
            _,_,d=nodes[k]
            # Start a path here. Birth cost discourages one-frame false tracks.
            best=BIRTH_PENALTY+SKIP_PENALTY
            pk=None
            for pt in range(ti-1,max(-1,ti-3),-1):
                ...
                for q in by_t.get(pt,()):
                    ...
                    if val<best:
                        best=val;pk=q
            dp[k]=best; prev[k]=pk
    # Prefer long, coherent tracks over isolated low-cost fragments.
    terminals=[]
    for k,cost in dp.items():
        ti,_,_=nodes[k]
        terminals.append((cost - 1.8*math.log1p(1+ti),k))
    _,k=min(terminals,key=lambda z:z[0])
    path=[]
    while k is not None:
        path.append(k); k=prev.get(k)
    path.reverse()
...
def extract_tracks(frames,n_tracks=10):
    ...
    for tid in range(n_tracks*2):
        ...
        if len(p)<6:break
        # Greedily remove nodes used by accepted paths. This prevents two
        # extracted trajectories from sharing the same fused detection.
...
def framewise_eval(frames,tb):
    ...
    for f in frames:
        if f['t']<=CAL:continue
        pred=np.asarray([d['xy'] for d in f['fused']],float) if f['fused'] else np.empty((0,2)); gt=truth_oos(tb,f['t'])
        gt_points+=len(gt); dets+=len(pred)
    ...
    return {'matched':matched,'gt_points':gt_points,'detections':dets,'recall_proxy':matched/max(1,gt_points),'precision_proxy':matched/max(1,dets),'mae_m':float(a.mean()) if len(a) else None,'rmse_m':float(np.sqrt(np.mean(a*a))) if len(a) else None,'p95_m':float(np.percentile(a,95)) if len(a) else None}

def calibration_mapping(tracks,tb):
    target=list(v10.TARGET_IDS); eligible=[]; tids=[]
    for tr in tracks:
        cal=[o for o in tr['obs'] if o['t']<=CAL]
        hold=[o for o in tr['obs'] if o['t']>CAL]
    ...
    for r,c in zip(ri,ci):
        mapping[tids[r]]=target[c]
        diag.append({'track_id':int(tids[r]),'gt_id':int(target[c]),'calibration_median_m':float(C[r,c])})
    return mapping,diag
...
def mapped_eval(tracks,mapping,tb):
    ...
    for tr in tracks:
        ...
        for o in tr['obs']:
            ...
            q=np.asarray([g['x'],g['y']],float);e=float(np.linalg.norm(o['xy']-q))
            rows.append({'t':o['t'],'track_id':tr['track_id'],'gt_id':gid,'pred_x':o['xy'][0],'pred_y':o['xy'][1],'truth_x':g['x'],'truth_y':g['y'],'position_error_m':e})
    a=np.asarray([r['position_error_m'] for r in rows],float)
    return {'mapped_tracks':len(mapping),'mapped_samples':len(rows),'mae_m':float(a.mean()) if len(a) else None,'rmse_m':float(np.sqrt(np.mean(a*a))) if len(a) else None,'p95_m':float(np.percentile(a,95)) if len(a) else None},rows

def anonymous_eval(tracks,tb):
    # Pure post-hoc diagnostic: the inference output remains anonymous. We assign
    # the closest GT label to each observed point only here, after construction.
    switches=0; matched=0; total=0; track_rows=[]
    for tr in tracks:
        labels=[]
        for o in tr['obs']:
            if o['t']<=CAL:continue
            gt=truth_oos(tb,o['t'])
            if not gt:continue
            best=min(((int(g['id']),float(np.linalg.norm(o['xy']-np.asarray([g['x'],g['y']])))) for g in gt),key=lambda z:z[1])
            labels.append(best)
        ...
        maj=Counter(l for l,_ in labels).most_common(1)[0][0]
        track_rows.append({'track_id':tr['track_id'],'samples':len(labels),'majority_gt_id':maj,'majority_fraction':sum(l==maj for l,_ in labels)/len(labels),'mean_nearest_error_m':float(np.mean([e for _,e in labels]))})

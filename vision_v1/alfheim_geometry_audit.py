...
from alfheim_benchmark_v9 import CameraModel, SEED, seed_model
...
def field_segments(n_circle: int = 96):
    ...
    def add(a, b, tag):
        s.append((np.asarray(a, float), np.asarray(b, float), tag))
    ...
    r = 9.15
    pts = np.column_stack(
        [52.5 + r * np.cos(np.linspace(0, 2 * math.pi, n_circle)),
         34.0 + r * np.sin(np.linspace(0, 2 * math.pi, n_circle))]
    )
    for a, b in zip(pts[:-1], pts[1:]):
        s.append((a, b, "center_circle"))
    return s
    ...
    for p in np.asarray(points, float):
        ...
        for a, b, tag in segments:
            ...
            if best is None or d < best:
                best = d
                best_tag = tag
        vals.append(float(best if best is not None else 999.0))
...
def detect_lines(frame):
    ...
    # Bright, elongated field markings are much more stable than player pixels.
    ...
    for raw in lines[:, 0]:
        ...
        # Ignore almost point-like and near-vertical image noise.
        if length < 90 or angle > math.radians(88):
            continue
...
def line_diagnostics(H, lines, segments):
    ...
    for seg in lines:
        ...
        if not np.all(np.isfinite(world)):
            continue
        # Reject wildly exploded projections; they usually come from Hough noise
        # outside the pitch ROI.
        if np.max(np.abs(world)) > 400:
            continue
        d, tags = nearest_field_line(world, segments)
        length = float(np.linalg.norm(world[1] - world[0]))
        rows.append(
            {
                "world_line_length_m": length,
                "endpoint_distance_a_m": float(d[0]),
                "endpoint_distance_b_m": float(d[1]),
                "mean_endpoint_distance_m": float(np.mean(d)),
                "line_tag_a": tags[0],
                "line_tag_b": tags[1],
            }
        )
...
def audit_video(path: str, cam: int, sample_seconds: float = 0.5):
    ...
    while t < max(0.001, duration):
        ...
        if not ok:
            break
        rows = line_diagnostics(H, detect_lines(frame), segments)
        all_rows.extend(rows)
        t += max(0.1, sample_seconds)
    cap.release()
    ...
    summary = {
        ...
        "seed_homography": np.asarray(H, float).tolist(),
        "mean_line_error_m": float(vals.mean()) if len(vals) else None,
        "median_line_error_m": float(np.median(vals)) if len(vals) else None,
        "p95_line_error_m": float(np.percentile(vals, 95)) if len(vals) else None,
        "good_line_fraction_under_2m": float(np.mean(vals <= 2.0)) if len(vals) else None,
        "note": "Image-only diagnostic. No ground truth is consumed.",
    }
...
def main():
    ...
    ap.add_argument("--sample-seconds", type=float, default=0.5)
    args = ap.parse_args()

    summary, rows = audit_video(args.video, args.cam, args.sample_seconds)
    out = Path(args.out)
    out.mkdir(parents=True, exist_ok=True)
    (out / f"cam{args.cam}_geometry.json").write_text(json.dumps(summary, indent=2))
    (out / f"cam{args.cam}_line_diagnostics.json").write_text(json.dumps(rows, indent=2))

...
def persist_track(frames):
    active=[];tent=[];all_tracks=[];nextid=0;prev_t=None
    for f in frames:
        ...
        for i,tr in enumerate(alltr):
            pred=tr.pred(t); proto=tr.proto(); gate=3.2 if tr.confirmed else 2.4; gate+=min(1.3,tr.miss*2.0)
        ...
        for j,d in enumerate(det):
            ...
            tr=Tr(nextid,[{'t':t,'xy':np.asarray(d['xy'],float),'feat':np.asarray(d['feat'],float),'cams':d['cams'],'conf':float(d['conf'])}],0.,1,False)
            nextid+=1; all_tracks.append(tr); tent.append(tr)
...
def eval_tracks(tracks,tb):
    ...
    mapping={}; mapdiag=[]
    if M:
        C=np.asarray(M,float); ri,ci=linear_sum_assignment(C)
        for r,c in zip(ri,ci):
            if C[r,c]<4.5:
                mapping[tids[r]]=target[c]; mapdiag.append({'track_id':tids[r],'gt_id':target[c],'calibration_median_m':float(C[r,c])})
    rows=[]
    for tr in candidates:
        ...
        for o in tr.obs:
            ...
            q=np.asarray([gt[gid]['x'],gt[gid]['y']],float); e=float(np.linalg.norm(o['xy']-q))
            rows.append({'t':o['t'],'track_id':tr.tid,'gt_id':gid,'pred_x':o['xy'][0],'pred_y':o['xy'][1],'truth_x':q[0],'truth_y':q[1],'position_error_m':e})
    a=np.asarray([r['position_error_m'] for r in rows],float)
    return {'candidate_tracks':len(candidates),'mapped_tracks':len(mapping),'mapped_samples':len(rows),'mae_m':float(a.mean()) if len(a) else None,'rmse_m':float(np.sqrt(np.mean(a*a))) if len(a) else None,'p95_m':float(np.percentile(a,95)) if len(a) else None,'mapping':mapping,'mapping_diagnostics':mapdiag},rows
...
def main():
    ...
    metrics.update({'version':'v53-oos-persistent-anonymous-redtrack','segment':'0059-0061','duration_s':dur,'sample_fps':SAMPLE_FPS,'calibration_seconds':CAL,'inference_uses_ground_truth':False,'confirmation_hits':CONFIRM_HITS,'tentative_max_age':TENTATIVE_MAX_AGE,'max_miss_s':MAX_MISS,'max_active_tracks':MAX_ACTIVE,'truth_usage':'evaluation only after anonymous tracks are constructed','all_track_histories_preserved':True,'all_track_count':len(tracks)})
    (out/'metrics.json').write_text(json.dumps(metrics,indent=2)); pd.DataFrame(rows).to_csv(out/'matched_observations.csv',index=False); pd.DataFrame([{'track_id':tr.tid,'samples':len(tr.obs),'confirmed':tr.confirmed,'last_t':tr.obs[-1]['t'] if tr.obs else None} for tr in tracks]).to_csv(out/'tracks_summary.csv',index=False); print(json.dumps(metrics,indent=2),flush=True)

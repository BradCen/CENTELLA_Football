...
from centella_analytics.events import DuelEvent, EventTimeline, PassEvent, SetPieceEvent, ShotEvent
...
def test_post_loss_pressure_and_regain():
    frames = [
        frame(10.0, [(40, 30), (50, 30), (60, 30)]),
        frame(11.0, [(41, 30), (51, 30), (60, 30)]),
    ]
...
def test_pitch_control_is_bounded_and_sums_to_100():
    players = [
        PlayerSample("h", "home", 30, 34, 0, vx=2),
        PlayerSample("a", "away", 75, 34, 0),
    ]
...
def test_xg_model_is_probabilistic():
...
def test_xa_duels_and_set_pieces():
    ...
    xA = expected_assists([p], [shot])
    assert xA["linked_passes"] == 1
